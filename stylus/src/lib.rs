...
impl SigmoidBondingCurve {
    ...
    pub fn initialize(&mut self, pool_id: B256, params: Bytes) -> Result<(), Vec<u8>> {
        ...
        if params_bytes.len() < 160 { // 5 * 32 bytes
        ...
        if total_supply.is_zero() || initial_price.is_zero() {
            return Err(Vec::<u8>::from("Invalid Parameters - zero values not allowed"));
        }
        
        // Use default values if not provided
        let max_price_factor = if max_price_factor.is_zero() { DEFAULT_MAX_PRICE_FACTOR } else { max_price_factor };
        let steepness = if steepness.is_zero() { DEFAULT_STEEPNESS } else { steepness };
        let midpoint = if midpoint.is_zero() { DEFAULT_MIDPOINT } else { midpoint };
        ...
        let sig = [
            0x9b, 0xf7, 0xf4, 0xad, 0x1d, 0x0f, 0x9f, 0xa8, 0xaa, 0x5c, 0x45, 0x69,
            0x13, 0xd7, 0x2b, 0x51, 0x41, 0x0f, 0x35, 0xa7, 0xc4, 0xcd, 0xf7, 0x34,
            0x94, 0xb7, 0xa8, 0x1b, 0x53, 0x0d, 0x7e, 0x40
        ];
    ...
    pub fn calculate_buy(&mut self, pool_id: B256, weth_amount: U256) -> Result<(U256, U256), Vec<u8>> {
        // Get pool info
        let (
            token_address,
            _creator,
        ...
        if circulating_supply.is_zero() {
            ...
            let mut topics = Vec::new();
            let sig = [
                0xb5, 0x76, 0x4e, 0x7b, 0x82, 0xdd, 0x8f, 0x30, 0x19, 0x96, 0xd3, 0x71,
                0x8c, 0xe0, 0xa3, 0x43, 0xf4, 0x74, 0xc9, 0x37, 0x93, 0xa6, 0xd3, 0x83,
                0xcb, 0x65, 0x6f, 0x91, 0x78, 0x69, 0xaf, 0xcf
            ];
            topics.push(B256::from_slice(&sig));
            topics.push(pool_id);
    ...
    pub fn calculate_sell(&mut self, pool_id: B256, token_amount: U256) -> Result<(U256, U256), Vec<u8>> {
        // Get pool info
        let (
            token_address,
            _creator,
        ...
        let weth_to_return = self.calculate_weth_for_token_amount(
            circulating_supply,
            token_amount,
            &params,
            true
        ...
        let mut topics = Vec::new();
        let sig = [
            0x6d, 0xfb, 0xff, 0xa4, 0x12, 0x55, 0xd2, 0x61, 0x0f, 0x46, 0xd2, 0x8a,
            0x68, 0xf7, 0xbb, 0xf0, 0xd3, 0xd0, 0x6a, 0xba, 0x0c, 0x73, 0x2c, 0x9a,
            0xdb, 0x02, 0xa9, 0x1f, 0x1b, 0xa5, 0xb7, 0x35
        ];
        topics.push(B256::from_slice(&sig));
        topics.push(pool_id);
    ...
    pub fn calculate_weth_for_exact_tokens(&mut self, pool_id: B256, exact_token_amount: U256) -> Result<(U256, U256), Vec<u8>> {
        let (
            token_address,
            _creator,
            _weth_collected,
    ...
    pub fn calculate_tokens_for_exact_weth(&mut self, pool_id: B256, exact_weth_amount: U256) -> Result<(U256, U256), Vec<u8>> {
        let (
            token_address,
            _creator,
            _weth_collected,
    ...
    pub fn set_pool_state_manager(&mut self, new_pool_state_manager: Address) -> Result<(), Vec<u8>> {
        self.only_owner()?;
    ...
    pub fn transfer_ownership(&mut self, new_owner: Address) -> Result<(), Vec<u8>> {
        ...
        let mut topics = Vec::new();
        let sig = [
            0x8b, 0xe0, 0x07, 0x9c, 0x53, 0x16, 0x59, 0x14, 0x13, 0x44, 0xcd, 0x1f,
            0xd0, 0xa4, 0xf2, 0x84, 0x19, 0x49, 0x7f, 0x97, 0x22, 0xa3, 0xda, 0xaf,
            0xe3, 0xb4, 0x18, 0x6f, 0x6b, 0x64, 0x57, 0xe0
        ];
        topics.push(B256::from_slice(&sig));
...
impl SigmoidBondingCurve {
    ...
    fn get_pool_info(&self, pool_id: B256) -> Result<(Address, Address, U256, U256, bool, B256), Vec<u8>> {
        // Create call data
        let mut call_data = Vec::with_capacity(36);
        ...
        if result.len() < 192 {
            return Err(Vec::<u8>::from("Invalid result length from pool state manager"));
        }
        ...
        Ok((token_address, creator, weth_collected, last_price, is_transitioned, bonding_curve_strategy))
    }
    ...
    fn calculate_sigmoid_price(&self, supply: U256, params: &CurveParameters) -> U256 {
        ...
        if percentage_sold < params.midpoint {
            ...
            params.initial_price.saturating_add(self.divide_fixed_point(price_range, denominator))
        } else {
            // Percentage_sold >= midpoint case
    ...
    fn exp_approx(&self, x: U256) -> U256 {
        ...
        for i in 1..15u64 { // Limit terms for performance
